...
def connect_users():
    return psycopg.connect(
        host="localhost", dbname="user_ui_db",
...
def register():
    ...
    if cur.fetchone() is not None:
        ...
        conn.close()
        return
    cur.execute("INSERT INTO users (Логин, Пароль) VALUES (%s, %s);", (login, password))
    conn.commit()
    cur.close()
    conn.close()
...
def open_books_window():
    ...
    entry_name = ttk.Entry(win); entry_name.pack(anchor=NW, padx=6, pady=2)
    ttk.Label(win, text="Автор").pack(anchor=NW, padx=6, pady=2)
    entry_author = ttk.Entry(win); entry_author.pack(anchor=NW, padx=6, pady=2)
    ttk.Label(win, text="Год").pack(anchor=NW, padx=6, pady=2)
    entry_year = ttk.Entry(win); entry_year.pack(anchor=NW, padx=6, pady=2)
    ttk.Label(win, text="Страниц").pack(anchor=NW, padx=6, pady=2)
    entry_pages = ttk.Entry(win); entry_pages.pack(anchor=NW, padx=6, pady=2)
    ttk.Button(win, text="Добавить", command=save_book).pack(anchor=NW, padx=6, pady=6)
    label = ttk.Label(win); label.pack(anchor=NW, padx=6, pady=6)
    win.mainloop()
...
entry_login = ttk.Entry(auth_window); entry_login.pack(anchor=NW, padx=6, pady=2)
ttk.Label(auth_window, text="Пароль").pack(anchor=NW, padx=6, pady=2)
entry_password = ttk.Entry(auth_window, show="*"); entry_password.pack(anchor=NW, padx=6, pady=2)
